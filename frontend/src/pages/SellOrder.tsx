  ...
  const handleSellOrder = async () => {
    try {
      ...
      if (axios.isAxiosError(error)) {
        alert(`Error placing sell order: ${error.response?.data?.message || "Unknown error"}`);
      } else {
      ...
      <div className="flex w-full p-5">
        ...
        {!loading && !error && (
          <div className="w-1/3 p-5 bg-white dark:bg-gray-900 rounded-lg shadow-lg">
            ...
            <button onClick={handleSellOrder} className="w-full py-3 bg-red-400 text-white font-semibold rounded-md">
              PLACE SELL ORDER
            </button>
          </div>
        )}
